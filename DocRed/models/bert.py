...
class Bert():
    ...
    def convert_tokens_to_ids(self, tokens, max_len, pad=True):
    ...
    def subword_tokenize(self, tokens, max_len):
        ...
        subwords = [self.CLS] + list(self.flatten(subwords))[:max_len-2] + [self.SEP]
        token_start_idxs = 1 + np.cumsum([0] + subword_lengths[:-1])
        token_start_idxs[token_start_idxs > max_len-3] = max_len-3
        return subwords, token_start_idxs

    def subword_tokenize_to_ids(self, tokens, max_len):
        """Segment each token into subwords while keeping track of
        ...
        """
        subwords, token_start_idxs = self.subword_tokenize(tokens, max_len)
        subword_ids, mask = self.convert_tokens_to_ids(subwords,max_len)
